...
class GraphWin(tk.Canvas):
    ...
    def getMouse(self):
        ...
        while self.mouseX == None or self.mouseY == None:
            self.update()
            if self.isClosed(): raise GraphicsError(
                "getMouse in closed window")
...
class GraphicsObject:
    ...
    def draw(self, graphwin):
        ...
        if self.canvas and not self.canvas.isClosed(): raise GraphicsError(
            OBJ_ALREADY_DRAWN)
        if graphwin.isClosed(): raise GraphicsError(
            "Can't draw to closed window")
